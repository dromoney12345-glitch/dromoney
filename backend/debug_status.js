...
try {
  ...
  function search(items) {
    items.forEach(i => {
      if(i.name && i.name.toLowerCase() === 'status') found.push(i);
      if(i.item) search(i.item);
    });
  }
  search(data.item);
  fs.writeFileSync('debug_status.txt', JSON.stringify(found, null, 2));
} catch (e) {
  fs.writeFileSync('debug_status.txt', e.toString());
}
