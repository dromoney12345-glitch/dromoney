                ...
                <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm p-8 flex flex-col relative group overflow-hidden">
                    ...
                    <div className="flex items-center justify-between mb-6 relative">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-sm font-medium text-slate-800 uppercase tracking-widest leading-none">Content Manager</h2>
                        </div>
                        <button onClick={() => setEditingCms(!editingCms)} className={`px-4 py-2.5 rounded-xl font-medium text-[10px] uppercase tracking-widest transition-all ${editingCms ? 'bg-emerald-500 text-white shadow-lg' : 'bg-slate-50 text-slate-500 border border-slate-100'}`}>{editingCms ? 'Save Changes' : 'Edit CMS'}</button>
                    </div>
                    <div className="space-y-6 relative">
                        <div>
                            <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2"><Info size={12} /> Description Text</p>
                        ...
                        <div>
                            <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2"><ShieldAlert size={12} /> Guidelines Box</p>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="text-sm font-medium text-slate-800 uppercase tracking-widest leading-none">Milestones & Today's Targets</h2>
                            </div>
                            <button 
                                onClick={() => {
                                    if (editingRules) {
                                ...
                                className={`px-4 py-2.5 rounded-xl font-medium text-[10px] uppercase tracking-widest transition-all ${editingRules ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-50 text-slate-400'}`}
                            >
                            ...
                            {[
                                ...
                                return (
                                    <div key={rule.key} className="bg-slate-50/50 border border-slate-100 p-4 rounded-2xl">
                                        <div className="flex items-center gap-2 mb-1.5"><Icon size={13} className={rule.color} /><p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest leading-none">{rule.label}</p></div>
                                        {editingRules ? (
                        ...
                        <div>
                            <h3 className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-3">Today's Activity Progress Targets</h3>
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                {[
                                    ...
                                    <div key={act.key} className="bg-slate-50/50 border border-slate-100 p-3.5 rounded-2xl">
                                        <p className="text-[8px] font-medium text-slate-400 uppercase tracking-wider mb-1.5 leading-none">{act.label}</p>
                                        {editingRules ? (
                                            <input 
                                                type="number" 
                                                value={rules[act.key]} 
            ...
            <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden mb-8">
                <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div><h2 className="text-sm font-medium text-slate-800 uppercase tracking-widest">User Progress Tracker</h2><p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter leading-none mt-1">Milestone Stages</p></div>
                    </div>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100">
                                <th className="text-left px-8 py-5 text-[10px] font-medium text-slate-400 uppercase tracking-[0.25em]">User</th>
                                <th className="text-left px-8 py-5 text-[10px] font-medium text-slate-400 uppercase tracking-[0.25em]">Progress</th>
                                <th className="text-left px-8 py-5 text-[10px] font-medium text-slate-400 uppercase tracking-[0.25em]">Stage</th>
                                <th className="text-center px-8 py-5 text-[10px] font-medium text-slate-400 uppercase tracking-[0.25em]">View History</th>
                            </tr>
                        </thead>
                            ...
                            {currentItems.length > 0 ? currentItems.map((u) => {
                                ...
                                return (
                                    <tr key={u.id} className="hover:bg-slate-50/40 transition-colors group">
                                        <td className="px-8 py-5"><span className="font-medium text-slate-800 text-[13px] tracking-tight">{u.name}</span></td>
                ...
                <div className="px-8 py-5 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between">
                    <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Showing {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredUsers.length)} of {filteredUsers.length}</p>
                ...
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-w-[380px] rounded-[32px] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-400">
                        <div className="bg-[#0F172A] p-6 text-white flex justify-between items-center">
                            <div><h3 className="text-md font-medium tracking-tight">{selectedUser.name}'s History</h3><p className="text-[9px] text-sky-400 font-medium uppercase tracking-widest leading-none mt-1">Last 7 Days Earnings</p></div>
                        ...
                        <div className="p-6">
                            {historyLoading ? (
                                <div className="flex flex-col items-center justify-center py-12 gap-3">
                                    ...
                                    <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Fetching transactions...</p>
                                </div>
                            ) : (
                                <div className="space-y-2 mb-6 max-h-[280px] overflow-y-auto scrollbar-hide">
                                    {selectedUserHistory.length > 0 ? selectedUserHistory.map((day, idx) => (
                                        ...
                                        <div className="py-10 text-center">
                                            <p className="text-[11px] font-medium text-slate-300 uppercase tracking-widest">No earnings in last 7 days</p>
                                        </div>
                                    )}
                                </div>
                            )}
                            <button onClick={() => { setSelectedUser(null); setSelectedUserHistory([]); }} className="w-full bg-[#0F172A] text-white py-4 rounded-[20px] font-medium text-[11px] uppercase tracking-widest shadow-xl active:scale-95 transition-all">Close History</button>
                        </div>
                    </div>
                </div>
            )}
